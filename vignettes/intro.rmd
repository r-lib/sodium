...
All Sodium functions operate on binary data, called 'raw' vectors in R. Use `charToRaw` and `rawToChar` to convert between strings and raw vectors. Alterantively `hex2bin` and `bin2hex` can convert between binary data to strings in hex notation:
...
Sodium has several hash functions including `hash()`, `shorthash()`, `sha256()`, `sha512` and `scrypt()`. The generic `hash()` is usually recommended. It uses [blake2b](https://download.libsodium.org/doc/hashing/generic_hashing.html) with a configurable size between 16 bytes (128bit) and 64 bytes (512bit).
