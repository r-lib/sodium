...
The sodium R package provides bindings to [libsodium](https://download.libsodium.org/doc/): a modern, easy-to-use software library for encryption, decryption, signatures, password hashing and more. 

The goal of Sodium is to provide all of the core operations needed to build higher-level cryptographic tools. It is not intended for implementing standardized protocols such as TLS, SSH or GPG. Sodium only supports a limited set of state-of-the-art elliptic curve methods, resulting in a simple but very powerful tool-kit for building secure applications.
...
All Sodium methods operate on binary data, which are called 'raw' vectors in R. With `charToRaw` and `rawToChar` we can convert between strings and raw vectors. Alterantively the Sodium package provides functions `hex2bin` and `bin2hex` to convert binary data to strings in hex notation:
...
Implementation is platform specific, see the [docs](https://download.libsodium.org/doc/generating_random_data/index.html) for  details.

### Hash functions

Sodium provices several hash functions including `hash()`, `shorthash()`, `sha256()`, `sha512` and `scrypt()`. The generic `hash()` is usually recommended, which implements [blake2b](https://download.libsodium.org/doc/hashing/generic_hashing.html), with a configurable size between 16 bytes (128bit) and 64 bytes (512bit).
...
The `shorthash()` function is a special 8 byte (64 bit) hash based on [SipHash-2-4](https://download.libsodium.org/doc/hashing/short-input_hashing.html). The output of this function is only 64 bits (8 bytes). It is useful for in e.g. Hash tables, but it should not be considered collision-resistant.


### Secret key encryption

Symmetric encryption uses the same secret key for both encryption and decryption. It is relatively simple and useful to encrypt local data, or as a building block for more complex methods.
...
Because the the secret has to be known by all parties, symmetric encryption by itself is often impractical for communication with third parties. For this we need asymmetric (public key) methods.

### Secret key authentication

Secret key authentication is called tagging in Sodium. A tag is basically a hashing of the data together with a secret key. 
...
To verify the integrity of the data at a later point in time, simply re-calculate the tag:
...
Asymmetric methods rely on public-private keypairs. Private keys are secret and should only be known to the owner. The public key on the other hand is not confidential and is shared with the other parties. Public keys are typically published on the users's website or posted in public directories or keyservers.
...
Signatures are used when the message itself is not confidential but integrity is important. A common use is for software repositories where to include an index file with checksums for all packages, signed by the repository maintainer. This allows client package managers to verify that the binaries were not manipulated by intermediate parties during the distribution process.
