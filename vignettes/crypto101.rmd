...
Now the `ciphertext` is an encrypted version of the message. Only if we know the `key` and the `nonce`, we can re-generate the keystream in order to `xor` the ciphertext back into the original message. 
...
This already provides a suitable basis for e.g. encrypting local data on disk. However because the same secret is used for both encryption and decryption, symmetric encryption by itself is impractical for communication. For exchanging secure messages with other parties, we need public key encryption.
