...
The logical [XOR operator](https://en.wikipedia.org/wiki/Exclusive_or#Truth_table) outputs `true` only when both inputs differ (one is `true`, the other is `false`). It is sometimes called an *invertor* because the output of `x` gets inverted if and only if `y` is true:
...
A *stream cipher* generates a unique stream of pseudo-random data based on a secret `key` and a unique `nonce`. Given the parameters it always generates the same stream. Sodium has a few stream ciphers:
...
We are not going to discuss implementation details, but if you think hard enough you could probably come up with something yourself. For example you could use a hash function such `sha256` and append counter which is incremented for each block (this is called CTR mode).
...
Each stream requires a `key` and a `nonce`. The key forms the shared secret and should only known to the trusted parties. The `nonce` is not secret and is stored or sent along with the ciphertext. The purpose of the `nonce` is to make a random stream unique to protect gainst re-use attacks. This way you can re-use a your key to encrypt multiple messages, as long as you never re-use the same nonce.

In practice, you should never write your own ciphers. A lot of [research](http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.58.1811&rep=rep1&type=pdf) goes into studying the properties of block ciphers under various modes of operation. In the remainder we just use the standard Sodium ciphers: `chacha20`, `salsa20`, `xsalsa20` or `aes128`.
...
We convert the password to a key of suitable size and use it to generate a random stream of equal length of the message:
...
Rather than using a single secret-key, assymetric (public key) encryption requires a *keypair*, consisting of a *public key* for encryption and a *private-key* for decryption. Data that is encrypted using a public key can only be decrypted using the corresponding private key. 
...
How does this work? Public key encryption makes use of [Diffie-Hellman](http://mathworld.wolfram.com/Diffie-HellmanProtocol.html): a method which allows two parties that have no prior knowledge of each other to jointly establish a shared secret key over an insecure channel. In the most simple case, both parties generate a temporary keypair and exchange their public key over the insecure channel. Diffie-Hellman allows both parties to calculate the (same) shared secret by combining their own private key with the other person's public key:
...
The classical [Diffie-Hellman method](http://mathworld.wolfram.com/Diffie-HellmanProtocol.html) is based on the discrete logarithm problem with very large prime numbers. Sodium uses [curve25519](http://cr.yp.to/ecdh/curve25519-20060209.pdf), a state-of-the-art function by Daniel Bernsteinan designed for use with the elliptic curve Diffie–Hellman (ECDH) key agreement scheme.
